...
The generator develops usable biographies, setting, relationships and opening scene. It treats explicit requirements as constraints, creatively fills open areas and leaves intentional mysteries unresolved. A separate `campaign.public_description` is intended for spoiler-free preview; the fuller `description` stays in author/GM context. The prompt is a creative brief, not a fact-extraction prompt. No purely structural validation can guarantee literary quality, so a real-model review remains important.
