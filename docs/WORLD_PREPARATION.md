...
Free idea, optional Scenario output, and legacy Markdown/text import converge in the editor. A new idea creates its preparation workspace automatically on submission. Quick creation invokes the existing versioned Scenario-writer prompt to develop the idea, then uses a second model request to distribute the resulting plan into structured World State. The original text remains a higher-priority constraint. If a prepared Scenario is selected, its plan goes straight to structured generation; import uses neither LLM request. Both calls use the user's chosen summary provider/model, are billed as separate `draft_scenario` and `draft_world` requests, and share the existing cancellation path. The intermediate plan is stored in the same draft document version and is available in Author View.
