...
Free idea, optional Scenario output, and legacy Markdown/text import converge in the editor. A new idea creates its preparation workspace automatically on submission. Quick creation sends the original idea straight to the structured-world generator in one model request: its creative brief both develops missing detail and builds World State. The separate “Со Сценаристом” route still generates a scenario and can pass it to the same world generator; the legacy Scenario → summary workflow remains available there. Import uses no LLM request. Draft generation uses the user's chosen summary provider/model and existing cancellation path; API usage is tracked as `draft_world`. The current job phase is persisted in the preparation job using an additive idempotent migration, so SSE reports generation progress. Older drafts with scenario outlines retain their version history; a newly generated world no longer creates or carries an intermediate outline.

The generator develops usable biographies, setting, directed relationships, existing conflicts, character knowledge and an opening scene in one generation stage, both when given a short idea and when given a prepared Scenario. A complete response normally needs one provider request; the existing continuation mechanism can make additional requests when output is truncated. No second quality-pass request is added. Explicit requirements are constraints; initial-generation gaps are creative space, while intentionally unresolved mysteries stay unresolved. This creative license never applies to runtime state updates. The earlier bundled prompt contained a contradictory instruction that allowed facts/events only from user input and a scaffold with empty relationship/thread maps; both are corrected. On startup the bundled prompt is automatically versioned over the previous untouched default, using `prompt_upgrades` so user-customized and restored prompts remain unchanged. A separate `campaign.public_description` is intended for spoiler-free preview; the fuller `description` stays in author/GM context.

After generation, the existing structural validation also runs an advisory initial-world coherence review (connections, motivations for relevant NPCs, scene/actor location and open lines). Warnings appear on the draft but do not block a valid world's confirmation; imported documents do not receive the creative checklist. This is deterministic and costs no extra model call. It cannot prove literary depth, preserve every subtle user constraint, or automatically improve an underperforming model's prose. Real DeepSeek/local-model quality still requires sampling; returning a syntactically valid but weak world remains possible.
