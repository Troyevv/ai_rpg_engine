...
Free idea, optional Scenario output, and legacy Markdown/text import converge in the editor. A new idea creates its preparation workspace automatically on submission. Quick creation invokes the existing versioned Scenario-writer prompt to develop the idea, then uses a second model request to distribute the resulting plan into structured World State. The original text remains a higher-priority constraint. If a prepared Scenario is selected, its plan goes straight to structured generation; import uses neither LLM request. Both calls use the user's chosen summary provider/model, are billed as separate `draft_scenario` and `draft_world` requests, and share the existing cancellation path. The current job phase is persisted in the preparation job using an additive idempotent migration, so SSE reports which step is running. The intermediate plan is stored in the same draft document version and is available in Author View.
...
Player View is projected server-side, excluding hidden world facts, other actors' intentions and arbitrary legacy root blobs. It displays the deliberately public campaign annotation rather than the GM description. Author View requires an explicit spoiler warning in the UI. This is a presentation boundary in the existing local single-user app, not a replacement authentication system.
