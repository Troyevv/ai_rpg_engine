...
def messages_for(repo,job,config):
    ...
    if task=='world':
        instructions=config['_draft_input']
...
def run(preparation,jid,config,api_key,handle,loaded,stream_fn):
    repo=preparation.repo;job=repo.preparation_job(jid);messages=messages_for(repo,job,config)
    original=repo.draft(job['workspace_id'],author=True)['state'];text='';last=0.0
    ...
    def generate(messages,limit):
        return tracked_stream(repo,stream_fn,jid,job['kind'],config,messages,model=config['model'],provider=config['provider'],api_key=api_key,
