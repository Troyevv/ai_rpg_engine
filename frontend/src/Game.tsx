            ...
            <>
              {save.turns.map((t, i) => (
                ...
                >
                  ...
                  />
                  {t.timing?.total!=null&&<details className="turn-timing"><summary>Время хода: {t.timing.total.toFixed(1)} с</summary><dl>{([['narrative','Художественная генерация'],['extraction','Извлечение состояния'],['extraction_repair','Исправление extraction'],['validation_apply','Валидация и применение'],['background_simulation','Фоновая симуляция'],['memory_compaction','Память'],['total','Всего']] as const).filter(([key])=>t.timing?.[key]!=null).map(([key,label])=><div key={key}><dt>{label}</dt><dd>{t.timing![key].toFixed(2)} с</dd></div>)}</dl></details>}
