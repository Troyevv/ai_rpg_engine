...
export function WorldCamera({save,open,onOpenChange,busy,observe,control}:{save:Save;open:boolean;onOpenChange:(v:boolean)=>void;busy:boolean;observe:(actor?:string,scene?:string)=>void;control:(id:string)=>void}){
 ...
 <div className="camera-tabs"><Button variant={tab==='camera'?'outline':'ghost'} onClick={()=>setTab('camera')}>Персонажи и сцены</Button><Button variant={tab==='timeline'?'outline':'ghost'} onClick={()=>setTab('timeline')}>Журнал событий</Button></div>
