...
export function RelationshipEditor({save,card,refresh}:{save:Save;card:Character;refresh:()=>void}){
 ...
 return <div className="motivation-editor">
  ...
  <div className="actions"><Button variant="ghost" disabled={busy} onClick={()=>setEdit(null)}>Отмена</Button><Button variant={edit?.delete?"danger":"primary"} disabled={busy||!edit?.target_id||(!edit.delete&&!edit.context.trim())} onClick={()=>void persist()}>{edit?.delete?'Удалить':'Сохранить'}</Button></div></DialogContent></Dialog>
 </div>
}
