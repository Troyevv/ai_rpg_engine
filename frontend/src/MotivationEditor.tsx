...
export function MotivationEditor({save,card,refresh}:{save:Save;card:Character;refresh:()=>void}){
 ...
 const persist=async()=>{
  ...
  const next=[...intentions];let short_goal=goal;
  if(entry.kind==='goal')short_goal=entry.remove?'':text.trim();
  else if(entry.remove)next.splice(entry.index,1);
  else if(entry.index<0)next.push(text.trim());else next[entry.index]=text.trim();
  try{
   await api(`/saves/${save.id}/characters/${encodeURIComponent(card.id)}/motivation`,{revision:save.revision,short_goal,intentions:next},'PATCH');
 ...
 const row=(value:string,kind:Entry['kind'],index:number)=><div className="motivation-row" key={`${kind}-${index}`}>
  <p>{value}</p><details className="motivation-menu"><summary aria-label={kind==='goal'?'Действия с целью':`Действия с намерением ${index+1}`}><MoreHorizontal size={20}/></summary><div>
 ...
 return <div className="motivation-editor">
  <h4>Текущая цель</h4>{goal?row(goal,'goal',0):<Button variant="ghost" onClick={()=>open({kind:'goal',index:-1})}>+ Добавить цель</Button>}
