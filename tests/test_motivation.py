...
def test_motivation_persists_isolated_and_enters_context(api):
    ...
    payload={'revision':save['revision'],'goals':['Проверить маяк','Вернуться домой'],'intentions':['Позвонить смотрителю']}
    response=client.patch(path,json=payload)
    assert response.status_code==200
    updated=response.json()
    fresh=client.get(f"/api/saves/{save['id']}").json()
    assert fresh['state']['world']['characters'][actor]['goals']==['Проверить маяк','Вернуться домой']
    ...
    payload.update(revision=updated['revision'],goals=[],intentions=[])
...
def test_motivation_authorization_validation_and_pov(api):
    ...
    body={'revision':save['revision'],'goals':['Цель'],'intentions':[]}
...
def test_motivation_does_not_rewrite_generation_snapshots(api):
    ...
    actor=save['state']['controlled_actor_id']
    assert client.patch(f"/api/saves/{save['id']}/characters/{actor}/motivation",json={'revision':save['revision'],'goals':['Новая цель'],'intentions':[]}).status_code==200
    with repo.connect() as db:
