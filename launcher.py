...
def frontend_digest():
    ...
    paths += sorted((FRONTEND / 'public').rglob('*'))
...
def start(open_browser=True):
    ...
    with (RUNTIME / 'launcher.lock').open('a+b') as lock:
        ...
        https=read_https()
        host=os.getenv('RPG_HOST','0.0.0.0' if https else '127.0.0.1')
        ...
        if https:
            with socket.socket() as probe:
                try:probe.bind(('0.0.0.0',https['port']))
                except OSError:raise RuntimeError(f"HTTPS port {https['port']} is busy.") from None
        env = {**os.environ, 'RPG_HOST':host, 'RPG_CONTROL_TOKEN': token, 'RPG_PORT': str(port), 'PYTHONUTF8': '1'}
        ...
        if os.name != 'nt':
            STATE.chmod(0o600)
        tls_context=ssl.create_default_context(cafile=str(RUNTIME/'tls'/'rootCA.pem')) if https else None
        for _ in range(150):
            if process.poll() is not None:
                raise RuntimeError('Server exited. See .runtime/server.log.')
            try:
                if request(url + '/api/health').get('status') == 'ready':
                    if https and request(f"https://127.0.0.1:{https['port']}/api/health",context=tls_context).get('status')!='ready':
                        continue
                    print('AI RPG Engine: ' + url)
                    if https:print(f"Phone / PWA: https://{https['ip']}:{https['port']}")
                    if open_browser:
                        webbrowser.open(url)
                    return
            except (OSError, ValueError):
...
def read_https():
    path=RUNTIME/'https.json'
    if not path.exists():return None
    value=json.loads(path.read_text(encoding='utf-8'))
    import ipaddress
    ipaddress.ip_address(value['ip'])
    if not isinstance(value['port'],int) or not 1024<=value['port']<=65535:raise RuntimeError('Invalid HTTPS port.')
    for name in ('server.pem','server.key'):
        if not (RUNTIME/'tls'/name).is_file():raise RuntimeError('Missing TLS certificate. Run setup_https.bat.')
    return value
...
def serve():
    ...
    app=create_app();https=read_https()
    host=os.getenv('RPG_HOST','0.0.0.0' if https else '127.0.0.1')
    servers=[uvicorn.Server(uvicorn.Config(app,host=host,port=int(os.getenv('RPG_PORT','8000')),
              log_level='info',timeout_graceful_shutdown=3,lifespan='off'))]
    if https:
        servers.append(uvicorn.Server(uvicorn.Config(app,host=host,port=https['port'],
            ssl_certfile=str(RUNTIME/'tls'/'server.pem'),ssl_keyfile=str(RUNTIME/'tls'/'server.key'),
            log_level='info',timeout_graceful_shutdown=3,lifespan='off')))
    def shutdown():
        for server in servers:server.should_exit=True
    app.state.shutdown=shutdown
    async def run():
        # One repository, one worker registry and one lifespan for both listeners.
...
if __name__ == '__main__':
    ...
    parser.add_argument('command', choices=['start', 'stop', 'serve','setup-https'], nargs='?', default='start')
    ...
    try:
        if args.command == 'setup-https':
            RUNTIME.mkdir(exist_ok=True)
            python=prepare()
            subprocess.run([str(python),str(ROOT/'local_https.py')],check=True,cwd=ROOT)
